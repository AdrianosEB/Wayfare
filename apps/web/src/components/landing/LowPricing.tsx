  ...
  return (
    <Section glow>
      ...
      <RevealGroup className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {trips.map((trip) => (
          // BudgetCard is shared with /pricing, so the reveal lives on a wrapper here rather
          // than being baked into the card itself.
          <motion.div key={trip.place} variants={revealItem}>
            <BudgetCard
              trip={trip}
